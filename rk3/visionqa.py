...
# the prescriber system prompt lives in prompts/vision-prescribe.system.md; it is
# loaded fresh at each prescribe() call (see below), so edits apply without a restart
...
def prescribe(slug, page, our_png=None, model=None):
    ...
    for attempt in range(2):
        try:
            res = vision_json(load_prompt("vision-prescribe.system.md"), user,
                              [orig, our_png], _PRESCRIBE_SCHEMA,
