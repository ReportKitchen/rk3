...
def _doc_warnings(ctx, pdf):
    ...
    if ("skia/pdf" in blob or "headlesschrome" in blob or "wkhtmltopdf" in blob
        ...
        warnings.append({
            "code": "web-print",
            "title": "Browser “Print to PDF” source",
            "detail": (
                f"This PDF was generated by a web browser’s print/export "
                f"(“{(producer or creator)[:60]}”), not a page-layout tool. "
                "These files flow tables and lists across page boundaries and "
                "can drop spaces between words, so reading order, tables, and "
                "spacing may be imperfect. This layout class isn’t a primary "
                "support target yet — RK3 converts it best-effort."),
        })
    for w in warnings:
